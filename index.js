...
rp(clevercarsURL)
    .then(function (html) {
        ...
        let makeModel = ($('h1>strong', html).text()).trim();
        let fuel_type = ($('li>.fa-tint', html)[0].next.data).trim();
        let mileage = ($('li>.fa-road', html)[0].next.data).trim();
        let car = { makeModel, fuel_type, mileage };
        fs.writeFile('resourses/$clevercars/data.json', JSON.stringify(car), (err) => {
            if (err) throw err;
        });
    })
    .catch(function (err) {
...
rp(autowellerURL)
    .then(function (html) {
        ...
        let car = {makeModel, fuel_type, mileage};
        fs.writeFile('resourses/$autoweller/data.json', JSON.stringify(car), (err) => {
            if (err) throw err;
        });
    })
    .catch(function (err) {
...
rp(autolandURL)
    .then(function (html) {
        const $ = cheerio.load(html)
        let makeModel = $('.product--title', html).last().text();
        let fuel_type;
        let mileage;
        let label;
        ...
        let car = { makeModel, fuel_type, mileage };
        fs.writeFile('resourses/$autoland/data.json', JSON.stringify(car), (err) => {
            if (err) throw err;
        });
    })
    .catch(function (err) {
